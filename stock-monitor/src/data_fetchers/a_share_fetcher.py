...
class AShareDataFetcher(BaseDataFetcher):
    ...
    def __init__(self):
        ...
        self._min_request_interval = 0.5  # akshare接口限速
    ...
    def get_sector_data(self, trade_date: Optional[str] = None) -> pd.DataFrame:
        ...
        """
        try:
            self.logger.info(f"正在获取A股板块资金流数据...")
            ...
            self.logger.info(f"成功获取 {len(df)} 条A股板块数据")
            return df
            
        except Exception as e:
            self.logger.error(f"获取A股数据失败: {str(e)}")
    ...
    def get_sector_historical(self, symbol: str, days: int = 30) -> pd.DataFrame:
        ...
        try:
            self._rate_limit()
            self.logger.info(f"正在获取板块 '{symbol}' 的历史数据...")
